...
for (ii=[0:len(piece_indices)-1]){
    ...
    Piece(indices, parity, sings, sing_directions, sing_locations, conn_size);
...
module Piece(indices, parity_of_piece, sings, directions, locations, conn_size){
    echo(indices)
    ...
    difference(){
        ...
        if (parity_of_piece==-1){
            socket_neg(direction, location, conn_size);
        }
        else if (parity_of_piece==1){
