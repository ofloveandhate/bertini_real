								...
								solver_configuration *solve_options)
{
	...
	isosingular_deflation(&num_deflations, &deflation_sequence, program_options->current_working_filename,
	...
	if (W.num_synth_vars==0) {
		...
		self_conjugate = checkSelfConjugate(W,num_vars,program_options->current_working_filename, program_options->stifle_text);  //later:  could be passed in from user, if we want
