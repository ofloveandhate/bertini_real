...
void WorkerSampleCurve(sampler_configuration & sampler_options, SolverConfiguration & solve_options)
{
	...
	sampler_configuration::Mode mode_to_run;
	MPI_Recv(&mode_to_run, 1, MPI_INT, solve_options.head(), CURVE, solve_options.comm(), MPI_STATUS_IGNORE);
	...
	switch (mode_to_run)
	{
		...
		case sampler_configuration::Mode::SemiFixed: // case for SemiFixed

		{
			C.SemiFixedSamplerWorker(V, sampler_options, solve_options);
			break;
		}
		case sampler_configuration::Mode::AdaptiveConsecDistance: //adaptive distance sampler
		{
			C.AdaptiveDistanceSamplerWorker(V, sampler_options, solve_options);
			break;
		}
		case sampler_configuration::Mode::AdaptivePredMovement: //adaptive movement sampler
		{
			C.AdaptiveMovementSamplerWorker(V, sampler_options, solve_options);
			break;
		}
...
void Curve::AdaptiveMovementSamplerSerial(VertexSet & V,
										   sampler_configuration & sampler_options,
										   SolverConfiguration & solve_options)
{
										   ...
										   SolverConfiguration & solve_options)
{
	...
	for (int ii=1; ii<solve_options.num_procs(); ii++) {
		sampler_configuration::Mode mode_to_run= sampler_configuration::Mode::AdaptivePredMovement;
		MPI_Send(&mode_to_run, 1, MPI_INT, ii, CURVE, solve_options.comm());

		V.send(ii, solve_options);
		this->send(ii, solve_options); // `this` is the curve
		//MPI_Send(&num_samples_per_interval[0], num_samples_per_interval.size(), MPI_INT, ii, DATA_TRANSMISSION, solve_options.comm());
...
void Curve::AdaptiveMovementSamplerWorker(VertexSet & V,
										   sampler_configuration & sampler_options,
										   SolverConfiguration & solve_options)
{
	...
	while (true) // while there is work to be done (an edge to sample)
	{
		...
		SampleEdgeAdaptiveMovement(edge_index, V, sampler_options, solve_options);
...
void Curve::AdaptiveDistanceSampler(VertexSet & V,   //when go into parallel or serial
													sampler_configuration & sampler_options,
													SolverConfiguration & solve_options)
{
	if (sampler_options.use_parallel())
...
void Curve::AdaptiveDistanceSamplerSerial(VertexSet & V,
													sampler_configuration & sampler_options,
													SolverConfiguration & solve_options)
{
													...
													SolverConfiguration & solve_options)
{
	...
	//seed the workers
	for (int ii=1; ii<solve_options.num_procs(); ii++) {
		sampler_configuration::Mode mode_to_run= sampler_configuration::Mode::AdaptiveConsecDistance;
		MPI_Send(&mode_to_run, 1, MPI_INT, ii, CURVE, solve_options.comm());
...
void Curve::AdaptiveDistanceSamplerWorker(VertexSet & V,
													sampler_configuration & sampler_options,
													SolverConfiguration & solve_options)
{
	if (!this->randomizer()->is_ready())
	...
	while (true) // while there is work to be done (an edge to sample)
	{
		...
		SampleEdgeAdaptiveDistance(edge_index, V, sampler_options, solve_options);
		ReportEdgeWorker(edge_index, V, solve_options);
	}
}
...
void Curve::SemiFixedSampler(VertexSet & V,   //create when go into parallel when into serial
									  sampler_configuration & sampler_options,
									  SolverConfiguration & solve_options,
...
void Curve::SemiFixedSamplerSerial(VertexSet & V,  //change this original one to be serial version
									  sampler_configuration & sampler_options,
									  SolverConfiguration & solve_options,
									  std::vector<int> const& num_samples_per_interval)
{
									  ...
									  std::vector<int> const& num_samples_per_interval)
{
	...
	//seed the workers
	for (int ii=1; ii<solve_options.num_procs(); ii++) {
		sampler_configuration::Mode mode_to_run= sampler_configuration::Mode::SemiFixed;
		MPI_Send(&mode_to_run, 1, MPI_INT, ii, CURVE, solve_options.comm());
	...
	// send workers num_samples_per_interval
...
void Curve::SemiFixedSamplerWorker(VertexSet & V,  //worker in parallel for semifixed
									  sampler_configuration & sampler_options,
									  SolverConfiguration & solve_options)
{
	if (!this->randomizer()->is_ready())
	...
	solve_options.get_PPD();
	MPI_Status status;
	std::vector<int> num_samples_per_interval; // will be resized and received into below
	int count; // will be populated by MPI_Get_count
	MPI_Probe(solve_options.head(), DATA_TRANSMISSION, solve_options.comm(), &status);
	MPI_Get_count(&status, MPI_INT, &count);
	num_samples_per_interval.resize(count); // resize the vector
	//MPI_Recv( &buf[0], count, MPI_INT, source, tag, MPI_COMM_WORLD, &status); //

	MPI_Recv(&num_samples_per_interval[0], count, MPI_INT, solve_options.head(), DATA_TRANSMISSION, solve_options.comm(), MPI_STATUS_IGNORE);
	...
	while (true) // while there is work to be done (an edge to sample)
	{
		...
		SampleEdgeSemiFixed(edge_index,V,sampler_options, solve_options, num_samples_per_interval);
		ReportEdgeWorker(edge_index, V, solve_options); //I don't think there needs to be any
		//changes to this call to accomodate SemiFixed
	}
}
...
void Curve::CycleNumSampler(VertexSet & V,
						  sampler_configuration & sampler_options,
						  SolverConfiguration & solve_options)
{
							  ...
							  int target_num_samples)
{
	...
	//seed the workers
	for (int ii=1; ii<solve_options.num_procs(); ii++) {
		sampler_configuration::Mode mode_to_run= sampler_configuration::Mode::Fixed;
		MPI_Send(&mode_to_run, 1, MPI_INT, ii, CURVE, solve_options.comm());
		V.send(ii, solve_options);
		this->send(ii, solve_options); // `this` is the curve
...
void Curve::CycleNumSamplerSerial(VertexSet & V,
							  sampler_configuration & sampler_options,
							  SolverConfiguration & solve_options)
{
	WitnessSet W;
	...
	CycleNumset_initial_sample_data();
	...
	for (unsigned int ii=0; ii<num_edges(); ii++) // for each of the edges
	{
		...
		SampleEdgeCycleNum(ii,V, sampler_options, solve_options);
							  ...
							  SolverConfiguration & solve_options)
{
	...
	//seed the workers
	for (int ii=1; ii<solve_options.num_procs(); ii++) {
		sampler_configuration::Mode mode_to_run= sampler_configuration::Mode::CycleNum;
		MPI_Send(&mode_to_run, 1, MPI_INT, ii, CURVE, solve_options.comm());
		V.send(ii, solve_options);
		this->send(ii, solve_options); // `this` is the curve
	}
...
void Curve::CycleNumSamplerWorker(VertexSet & V,
									  sampler_configuration & sampler_options,
									  SolverConfiguration & solve_options)
{
	if (!this->randomizer()->is_ready())
	...
	while (true) // while there is work to be done (an edge to sample)
	{
		...
		SampleEdgeCycleNum(edge_index, V, sampler_options, solve_options);
		ReportEdgeWorker(edge_index, V, solve_options);
	}
}
...
void Curve::SampleEdgeCycleNum(	int ii,
								VertexSet & V,
								sampler_configuration & sampler_options,
								SolverConfiguration & solve_options)
{
	...
	auto startpt_index = get_edge(ii).midpt();
	auto left_index = get_edge(ii).left();
	auto right_index = get_edge(ii).right();
	...
	// set up the slicing linear to have correct value
	vec_mp start_projection;
	init_vec_mp(start_projection,num_vars);  start_projection->size = num_vars;
	vec_cp_mp(start_projection,pi(0)); // grab the projection, copy it into start_projection
	neg_mp(&(start_projection->coord[0]), &(V[startpt_index].projection_values())->coord[curr_proj_index]);


	// set the point and the linear into the witness_set for multilin tracking
	set_witness_set_mp(W, start_projection, V[startpt_index].point()); // set the witness point and linear in the input for the lintolin solver.
	...
	// make a linear for tracking *towards*.  only the first coordinate later needs to be set, cuz the linear is constant for this solve
	vec_mp target_projection;
	init_vec_mp(target_projection,num_vars); target_projection->size = num_vars;
	vec_cp_mp(target_projection,pi(0)); // copy the projection into target_projection
	...
	comp_mp temp, temp1, target_projection_value; init_mp(temp);  init_mp(temp1); init_mp(target_projection_value);
	comp_mp temp_real; init_mp(temp_real); set_zero_mp(temp_real); // do not ever make the imaginary part of this non-zero
	comp_mp half; init_mp(half); mpf_set_d(half->r, 0.5); mpf_set_d(half->i, 0.0);

	Vertex temp_vertex;

	// the indices of the points computed in this function.  they'll be swapped into the real place they go later in the function
	std::vector<int> current_indices;

	// config for the solver
	MultilinConfiguration ml_config(solve_options, this->randomizer());

	// unpack the cycle numbers for this edge. 
	auto cycle_num_left = this->GetMetadata(ii).CycleNumLeft();
	auto cycle_num_right = this->GetMetadata(ii).CycleNumRight();
	...
	dehomogenize(&dehom_left,V[left_index].point());
	dehomogenize(&dehom_right,V[startpt_index].point());

	// compute how far apart the left and mid points are
	...
	int num_samples_left_side = std::max(ceil(temp_d->r*1.41421356), floor(sampler_options.min_num_samples_per_rib/2) ); // estimate the number of points on this half-edge

	dehomogenize(&dehom_left,V[startpt_index].point()); 
	dehomogenize(&dehom_right,V[right_index].point()); 
	...
	int num_samples_right_side = std::max(ceil(temp_d->r*1.41421356), floor(sampler_options.min_num_samples_per_rib/2) ); // estimate the number of points on this half-edge

	// note:
	// num_samples_right_side and num_samples_left_side are the number IN THE EDGE, not including the midpoint, or the sample points on the left or right endpoints


	// w = 1/2.  THIS ASSUMES THE MIDPOINT IS 1/2 WAY BETWEEN IN PROJECTION SPACE.
	comp_mp interval_width_u; init_mp(interval_width_u); 
	set_mp(interval_width_u, half);

	// n = num_samples+1
	comp_mp num_intervals;  init_mp2(num_intervals,1024); set_zero_mp(num_intervals);
	mpf_set_d(num_intervals->r,num_samples_left_side+1);

	// w = w/n
	div_mp(interval_width_u,interval_width_u,num_intervals);

	// u = 0, starting from the left.
	comp_mp curr_u_val; init_mp(curr_u_val); set_zero_mp(curr_u_val);



	current_indices.push_back(left_index);

	comp_mp scaled_u_val; init_mp(scaled_u_val); // a temp

	comp_mp proj_val_left; init_mp(proj_val_left);   // fixed for the duration of this sampling routine
	comp_mp proj_val_right; init_mp(proj_val_right); // fixed for the duration of this sampling routine

	set_mp(proj_val_left, &V[left_index].projection_values()->coord[curr_proj_index]);   // fixed for the duration of this sampling routine
	set_mp(proj_val_right, &V[right_index].projection_values()->coord[curr_proj_index]); // fixed for the duration of this sampling routine 

	
	for (int rr=0; rr<num_samples_left_side; ++rr){

		// increment to the next UNSCALED projection value (between 0 and 1)
		add_mp(curr_u_val, curr_u_val, interval_width_u);

		// gives a number between 0 and 1 (on bottom half, should be between 0 and 1/2)
		ScaleByCycleNum(scaled_u_val, curr_u_val, cycle_num_left, cycle_num_right);
		...
		// target_projection->coord[0] = scaled_u_val*(proj_val_right - proj_val_left) + proj_val_left

		sub_mp(target_projection_value, proj_val_right, proj_val_left);
		mul_mp(target_projection_value, scaled_u_val, target_projection_value);
		add_mp(target_projection_value, target_projection_value, proj_val_left);

		// finally, set the target projection value into the linears for the solve.
		neg_mp(&target_projection->coord[0],target_projection_value);
	...
	current_indices.push_back(startpt_index);

	// AND THEN THE RIGHT HALF


	// n = num_samples+1
	mpf_set_d(num_intervals->r,num_samples_right_side+1);

	// w = (1/2) / n
	div_mp(interval_width_u,half,num_intervals);

	// u = 1/2.  we'll add to it iteratively
	set_mp(curr_u_val,half);
	for (int rr=0; rr<num_samples_right_side; ++rr){
	...
	current_indices.push_back(right_index);


	sample_indices_[ii].swap(current_indices);

	if (sampler_options.verbose_level()>=2)
		printf("done cycle-num sampling edge\n");
    ...
    clear_mp(interval_width_u); clear_mp(num_intervals); clear_mp(curr_u_val); clear_mp(scaled_u_val);
    clear_mp(proj_val_left); clear_mp(proj_val_right); clear_mp(temp_real); clear_mp(half);
...
int  Curve::CycleNumset_initial_sample_data()
{
