									  ...
									  SolverConfiguration & solve_options)
{
	...
	std::cout << "fixed sampler worker" << std::endl;
	...
	while (1) // while there is work to be done (an edge to sample)
	{
		...
		if (sampler_options.verbose_level() >= 0)
			std::cout << "\tworker " << solve_options.id() << " sampling edge " << edge_index << std::endl;
...
{
	std::cout << "report edge worker" << std::endl;
	// sam, here
