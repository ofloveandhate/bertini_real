										...
										SolverConfiguration & solve_options)
{
	...
	const Curve* top = curve_with_name(curr_face.system_name_bottom());
	const Curve* bottom = curve_with_name(curr_face.system_name_top());;
	...
	for (int zz=0; zz<int(ribs[0].size())-1; zz++) {
		if (mpf_cmp((V[ribs[0][zz]].projection_values())->coord[1].r, (V[ribs[0][zz+1]].projection_values())->coord[1].r) > 0) {
			std::cout << "out of order, cuz these are off:" << std::endl;
			print_comp_matlab((V[ribs[0][zz]].projection_values())->coord,"l");
			print_comp_matlab((V[ribs[0][zz+1]].projection_values())->coord,"r");
		}
	}
	
	for (int zz=0; zz<int(ribs[target_num_samples-1].size())-1; zz++) {
		if (mpf_cmp((V[ribs[target_num_samples-1][zz]].projection_values())->coord[1].r, (V[ribs[target_num_samples-1][zz+1]].projection_values())->coord[1].r) > 0) {
			std::cout << "out of order, cuz these are off:" << std::endl;
			print_comp_matlab((V[ribs[target_num_samples-1][zz]].projection_values())->coord,"l");
			print_comp_matlab((V[ribs[target_num_samples-1][zz+1]].projection_values())->coord,"r");
		}
	}
	...
	for (int jj=1; jj<target_num_samples-1; jj++) {
		...
		try {
			ribs[jj].resize(3);
			ribs[jj][0] = bottom->sample_index(curr_face.bottom_edge(),jj);
			ribs[jj][2] = top->sample_index(curr_face.top_edge(),jj);			
		...
		int curr_bottom_index = ribs[jj][0];
		int curr_top_index = ribs[jj][2];
		...
		if (success_indicator!=SUCCESSFUL) {
			...
			if (W_new.num_points()==0) {
				...
				if (sampler_options.no_duplicates){
					ribs[jj][1] = index_in_vertices_with_add(V, temp_vertex);
				}
				else
				{
					ribs[jj][1] = V.add_vertex(temp_vertex);
				}
				{
					// copy in the start point for the multilin method, as the terminal point from the previous call.
					vec_cp_mp(W_multilin.point(0),V[ribs[jj][1]].point());
					W_multilin.point(0)->size = this->num_variables();
					neg_mp(&W_multilin.linear(0)->coord[0],&(V[ribs[jj][1]].projection_values())->coord[0]);
					neg_mp(&W_multilin.linear(1)->coord[0],&(V[ribs[jj][1]].projection_values())->coord[1]);
