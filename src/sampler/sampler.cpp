										   ...
										   SolverConfiguration & solve_options)
{
	...
	for (unsigned int ii=0; ii<num_edges(); ii++) // for each of the edges
	{
		...
		prev_num_samp = num_samples_each_edge_[ii]; // grab the number of points from the array of integers
		...
		while (1) // breaking condition is all samples being less than TOL away from each other (in the infty norm sense).
		{
			...
			if( (num_refinements == 0) || (pass_number >= sampler_options.maximum_num_iterations) ) // if have no need for new samples
			{
				...
				num_samples_each_edge_[ii] = sample_counter;
...
int  Curve::adaptive_set_initial_sample_data()
{
	...
	num_samples_each_edge_.resize(num_edges());
	
	for (unsigned int ii=0; ii<num_edges(); ii++){
		num_samples_each_edge_[ii]=3; // left, right, mid
	}
...
int  Curve::fixed_set_initial_sample_data(int target_num_samples)
{
	...
	for (unsigned int ii=0; ii<num_edges(); ii++){
		if (get_edge(ii).is_degenerate()) {
			num_samples_each_edge_[ii]=1;
		}
		else{
			num_samples_each_edge_[ii]=target_num_samples; // left, right, mid
		}
	...
	for (unsigned int ii=0; ii<num_edges(); ii++)
	{
		sample_indices_[ii].resize(num_samples_each_edge_[ii]);
		if (num_samples_each_edge_[ii]==1) {
			sample_indices_[ii][0] = get_edge(ii).midpt();
		}
		else
		{
			sample_indices_[ii][0] = get_edge(ii).left();
...
void  Curve::output_sampling_data(boost::filesystem::path base_path)
{
										...
										SolverConfiguration & solve_options)
{
	...
	for (unsigned int ii=0; ii<num_faces(); ii++) {
		...
		if (faces_[ii].is_degenerate()) {
			continue;
		}
		
		
		int slice_ind = faces_[ii].crit_slice_index();
		

		Curve & current_midslice = mid_slices_[slice_ind];
		Curve & left_critslice = crit_slices_[slice_ind];
		Curve & right_critslice = crit_slices_[slice_ind+1];
		...
		md_config.system_name_top = faces_[ii].system_name_top();
		md_config.system_name_bottom = faces_[ii].system_name_bottom();
		
		
		bottom_ = curve_with_name(faces_[ii].system_name_bottom());
		top_ = curve_with_name(faces_[ii].system_name_top());
		...
		for (int kk=0; kk<this->num_variables(); kk++) {
			set_mp(&W_midtrack.point(0)->coord[kk], &(V[faces_[ii].midpt()].point())->coord[kk]);
			var_counter++;
		}
		
		int mid_edge = current_midslice.edge_w_midpt(faces_[ii].midpt());
		// bottom
		...
		for (unsigned int jj=0; jj<faces_[ii].num_left(); jj++) {
			int asdf = faces_[ii].left_edge(jj);
			
			for (unsigned int kk = 0; kk< left_critslice.num_samples_on_edge(asdf); kk++) {
				if (jj>0 && kk==0) {
					if (left_critslice.sample_index(asdf,kk)==rib_indices[0].back()) {
						continue;
					}
					
				}
				
				rib_indices[0].push_back(left_critslice.sample_index(asdf,kk));
				
			}
			
		}
			
		for (unsigned int jj=0; jj<faces_[ii].num_right(); jj++) {
			int asdf = faces_[ii].right_edge(jj);
			for (unsigned int kk = 0; kk< right_critslice.num_samples_on_edge(asdf); kk++) {
				
				if (jj>0 && kk==0) {
					if (right_critslice.sample_index(asdf,kk)==rib_indices[target_num_samples-1].back()) {
						continue;
					}
					
				}
				
				rib_indices[target_num_samples-1].push_back(right_critslice.sample_index(asdf,kk));
			}
		}
...
void  Surface::output_sampling_data(boost::filesystem::path base_path)
{
