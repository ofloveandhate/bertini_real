...
int  curve_decomposition::adaptive_set_initial_sample_data()
{
	...
	for (unsigned int ii=0; ii<num_edges(); ii++)
	{
		...
		sample_indices[ii][0] = get_edge(ii).left();
		sample_indices[ii][1] = get_edge(ii).midpt();
		sample_indices[ii][2] = get_edge(ii).right();
...
int  curve_decomposition::fixed_set_initial_sample_data(int target_num_samples)
{
	...
	for (unsigned int ii=0; ii<num_edges(); ii++)
	{
		...
		if (num_samples_each_edge[ii]==1) {
			sample_indices[ii][0] = get_edge(ii).midpt();
		}
		else
		{
			sample_indices[ii][0] = get_edge(ii).left();
			
			sample_indices[ii][target_num_samples-1] = get_edge(ii).right();
		}
