...
int main(int argC, char *args[])
{
	...
	samplingNamenew = directoryName / "samp.curvesamp";
	...
	switch (dimension) {
		case 1:
		{
			...
			//output
			C.output_sampling_data(V, samplingNamenew);
							...
							solver_configuration & solve_options)
{
	...
	std::cout << D.input_filename << std::endl;
									  ...
									  int target_num_samples)
{
	...
	W.reset_patches(); // is this necessary?  i don't think so, only if we actually parse from file and get patches as a result.  then again, it doesn't hurt.
	for (int ii=0; ii<num_patches; ii++) {
		W.add_patch(patch[ii]);
	}
	...
	W.add_linear(pi[0]); // grab the projection, copy it into start_projection
	...
	comp_mp interval_width; init_mp(interval_width); set_zero_mp(interval_width);
	comp_mp num_intervals;  init_mp(num_intervals); set_zero_mp(num_intervals);
...
void  curve_decomposition::output_sampling_data(const vertex_set & V,
											  boost::filesystem::path samplingName)
{

	
	V.print(samplingName.parent_path() / "V.vertex");
