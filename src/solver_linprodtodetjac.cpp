...
//	write_linears(*W_new,"rand_comp_proj");
	...
	if (MPType==1){
		linprod_to_detjac_solver_mp(MPType,W,randomizer_matrix_full_prec,projection_full_prec,
																W_new,solve_options);
	}
	else{
		linprod_to_detjac_solver_d( MPType,W,randomizer_matrix_full_prec,projection_full_prec,
															 W_new,solve_options);
	}
	...
	if (solve_options.complete_witness_set==1){
		...
		W_new->num_linears = (1);
		W_new->L_mp = (vec_mp *)br_malloc(1*sizeof(vec_mp));
		
		init_vec_mp2(W_new->L_mp[0],W.num_variables,1024); //the 1024 here is incorrect
...
{ // need to create the homotopy
  ...
  *midOUT = safe_fopen_write(midName);
  ...
  if (T->MPType == 2) // using AMP - need to allocate space to store BED_mp
    ED->BED_mp = (linprodtodetjac_eval_data_mp *)br_malloc(1 * sizeof(linprodtodetjac_eval_data_mp));
  else
    ED->BED_mp = NULL;
  ...
  rank = rank_finder_d(&ED->preProcData, dummyProg, T, T->numVars);
	...
	setuplinprodtodetjacEval_d(T,const_cast<char *>(preprocFile.c_str()), const_cast<char *>(degreeFile.c_str()), dummyProg, rank, patchType, ssType, T->MPType, &T->numVars, NULL, NULL, NULL, ED, adjustDegrees, randomizer_matrix_full_prec, W,projection,solve_options);
...
//	printf("t = %lf+1i*%lf;\n", pathVars->r, pathVars->i);
  ...
  Jp->cols = 1;
	...
	for (jj=0; jj<BED->randomizer_matrix->rows; jj++) {
	...
	set_d(&funcVals->coord[BED->randomizer_matrix->rows],detjac);
	mul_d(temp,detjac,one_minus_s);//  (1-s)*detjac
	add_d(&funcVals->coord[BED->randomizer_matrix->rows],temp,linprod_times_gamma_s);
	...
	int offset = BED->num_variables-BED->patch.num_patches;
  ...
  for (ii = 0; ii < BED->randomizer_matrix->rows; ii++)
	...
	for (jj=0; jj<BED->patch.num_patches; jj++) {
		for (ii=0; ii<BED->num_variables; ii++) {
			set_d(&Jv->entry[jj+offset][ii],&Jv_Patch->entry[jj][ii]);
		}
	}
	...
	add_d(&Jp->entry[BED->randomizer_matrix->rows][0],temp2,temp);
																	 ...
																	 linprodtodetjac_eval_data_d **BED_copy, linprodtodetjac_eval_data_d *ED_d, linprodtodetjac_eval_data_mp *ED_mp)
{
  ...
  *NONSOLN_copy = (FILE **)br_malloc(max_threads * sizeof(FILE *));
  ...
  { // allocate memory
    ...
    *BED_copy = (linprodtodetjac_eval_data_d *)br_malloc(max_threads * sizeof(linprodtodetjac_eval_data_d));
...
//	printf("setup linprodtodetjac_d 1323\n");
  ...
  generic_setup_patch(&BED->patch,W);
	...
	for (ii=0; ii<W.num_linears; ++ii) {
		init_vec_d(BED->linears[ii],W.L_mp[ii]->size); BED->linears[ii]->size = W.L_mp[ii]->size;
		vec_mp_to_d(BED->linears[ii], W.L_mp[ii]);
	}
  ...
  { // using AMP - initialize using 16 digits & 64-bit precison
    ...
    int digits = 16, prec = 64;
		...
		for (ii=0; ii<W.num_linears; ++ii) {
			init_vec_mp2(BED->BED_mp->linears[ii],W.L_mp[ii]->size,prec);  // copy in the limited-precision linears
			BED->BED_mp->linears[ii]->size = W.L_mp[ii]->size;
			vec_cp_mp(BED->BED_mp->linears[ii], W.L_mp[ii]);
			
			init_vec_mp2(BED->BED_mp->linears_full_prec[ii],W.L_mp[ii]->size,T->AMP_max_prec);  //copy in the full precision linears
			BED->BED_mp->linears_full_prec[ii]->size = W.L_mp[ii]->size;
...
void cp_linprodtodetjac_eval_data_d(linprodtodetjac_eval_data_d *BED, linprodtodetjac_eval_data_d *BED_d_input, linprodtodetjac_eval_data_mp *BED_mp_input, int MPType)
{
  ...
  { // need to also setup MP versions since using AMP
    BED->BED_mp = (linprodtodetjac_eval_data_mp *)br_malloc(1 * sizeof(linprodtodetjac_eval_data_mp));
...
{ // need to create the homotopy
  ...
  *midOUT = safe_fopen_write(midName);
  ...
  numGps = ED->preProcData.num_var_gp + ED->preProcData.num_hom_var_gp;
	...
	ssType = 0;    // with 1-hom, we use total degree start system
	adjustDegrees = 0; // if the system does not need its degrees adjusted, then that is okay
	setuplinprodtodetjacEval_mp(const_cast<char *>(preprocFile.c_str()), const_cast<char *>(degreeFile.c_str()), dummyProg, rank, patchType, ssType, T->Precision, &T->numVars, NULL, NULL, NULL, ED, adjustDegrees, randomizer_matrix, W,projection_full_prec,solve_options);
...
//	}
//	print_matrix_to_screen_matlab(tempmat,"detme");
//	mypause();
	//now TAKE THE DETERMINANT of tempmat.
	take_determinant_mp(detjac,tempmat); // the determinant goes into detjac
//	print_comp_matlab(detjac,"detjac");
...
//	print_comp_matlab(one_minus_s,"one_minus_s");
//	print_comp_matlab(pathVars,"path");
//	print_point_to_screen_matlab(funcVals,"f_maybe");
...
//	print_comp_matlab(pathVars,"pathvars");
//	print_matrix_to_screen_matlab( AtimesJ,"jac");
//	print_point_to_screen_matlab(current_variable_values,"currvars");
//	print_matrix_to_screen_matlab(Jv_detjac,"Jv_detjac");
//	print_point_to_screen_matlab(linprod_derivative,"linprod_derivative");
																		...
																		linprodtodetjac_eval_data_mp **BED_copy, linprodtodetjac_eval_data_mp *ED_mp)
{
  ...
  *NONSOLN_copy = (FILE **)br_malloc(max_threads * sizeof(FILE *));
  ...
  { // allocate memory
    ...
    *trackCount_copy = (trackingStats *)br_malloc(max_threads * sizeof(trackingStats));
    *T_copy = (tracker_config_t *)br_malloc(max_threads * sizeof(tracker_config_t));
    *BED_copy = (linprodtodetjac_eval_data_mp *)br_malloc(max_threads * sizeof(linprodtodetjac_eval_data_mp));
																 ...
																 solver_configuration & solve_options)
{
  ...
  setupPreProcData(preprocFile, &BED->preProcData);
	...
	for (ii=0; ii<W.num_linears; ++ii) {
		init_vec_mp(BED->linears[ii],W.L_mp[ii]->size); BED->linears[ii]->size = W.L_mp[ii]->size;
...
//		{
//			printf("got not a number\n");
//			print_point_to_screen_matlab(EG->PD_mp.point,"bad_solution");
//      return 0;
//		}
//	}
//	
...
//		print_point_to_screen_matlab(LPED->linears[ii],"l");
//	}
	
	
	
	
	print_comp_matlab(zerotime,"t");
	
//	dot_product_mp(result,LPED->linears[0],current_values);
//	print_comp_matlab(result,"dot_p");
	print_comp_matlab(LPED->gamma,"gamma");
	print_point_to_screen_matlab(current_values,"currvals");
	print_point_to_screen_matlab(LPED->projection,"proj");
	print_point_to_screen_matlab(e_d.funcVals,"f");
	print_matrix_to_screen_matlab(e_d.Jv,"Jv");
...
//	print_comp_matlab(lambda,"lambda");
//	print_point_to_screen_matlab(e_d2.funcVals,"f2");
//	print_matrix_to_screen_matlab(e_d2.Jv,"Jv2");
