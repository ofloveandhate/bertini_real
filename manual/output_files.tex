...
FOR // each vertex
	...
	input_filename_index    // this is the first one
	FOR each filename this point was found in
		input_filename_index // all on one line
	END FOR
	vertex_type  // see file vertex_types.  bits represent types.

	num_paths_ending_here
	FOR each path ending here
		path_num
	END FOR
END FOR
\end{lstlisting}
...
The vertex types are represented by powers of 2, so that you can use bit ops to see all the types.
