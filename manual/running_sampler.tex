...
First, choose the curve you wish to produce. (In this case I am choosing the {\tt eistute\_sphere}, which is found in the {\tt intersections} file which can be found in the {\tt zoo} file) & \includegraphics[width=0.4\textwidth]{curvesampler1}  \\ \hline 
Invoke {\tt bertini} and {\tt bertini\_real} by entering in each on the command line individually & \includegraphics [width=0.4\textwidth]{curvesampler2} \\ \hline 
Invoke {\tt sampler} by entering it in on the command line & \includegraphics [width=0.4\textwidth]{curvesampler3} \\ \hline  
Now use Matlab to produce the image of the curve.
1. Go to the folder that holds your curve, then type {\tt gather\_br\_samples}
2. To produce the image, type in {\tt bertini\_real\_plotter}
...
{\tt bertini}, {\tt bertini\_real}, {\tt sampler}. 
& \includegraphics[width=0.4\textwidth]{surfacesampler1}  \\ \hline 
Once sampler has been invoked, go to matlab and follow the same steps layed out for the sampler curve process, by gathering and plotting. Depending on refinements, you will end up with a sphere with fewer or more points. Here are the two matlab commands: {\tt gather\_br\_samples}, {\tt bertini\_real\_plotter}.
